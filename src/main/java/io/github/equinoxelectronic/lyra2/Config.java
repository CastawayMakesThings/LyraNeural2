package io.github.equinoxelectronic.lyra2;

public class Config {
    //Should the logging be verbose
    public static boolean VERBOSE = false;
    //The version of Lyra2
    public static final String version = "1.0.0";
    //The version of the parser/serializer
    public static final String fileVersion = "1.0.0";
    //The header for lyra files
    public static final String lyraFileHeader = "LyraNeural2 by Equinox Electronic v"; // + fileVersion
}

//This is a general settings class. As you can see, there
//are not that many values in, but I hope that will change soon.

//Equinox Electronic